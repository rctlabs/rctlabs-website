...
export default async function MethodologyPage() {
  ...
  const pillars = [
    ...
    },
    {
      ...
      titleTh: "ขอบเขตการเปิดเผยข้อมูล",
      body: isTh
    ...
    },
    {
      ...
      titleTh: "การตรวจทานและการอัปเดต",
      body: isTh
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto max-w-6xl px-4 py-24 md:py-32">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold text-accent">{isTh ? "ระบบความน่าเชื่อถือด้านความรู้" : "Knowledge trust system"}</p>
          <h1 className="mt-3 text-4xl md:text-5xl font-bold text-foreground">{isTh ? "วิธีวิทยา" : "Methodology"}</h1>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {pillars.map((pillar) => (
            <div key={pillar.title} className="rounded-2xl border border-border bg-card p-6">
              <h2 className="text-xl font-semibold text-foreground">{isTh ? pillar.titleTh : pillar.title}</h2>
        ...
        <div className="mt-10 rounded-2xl border border-border bg-card p-8">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <Link href={`${localePrefix}/editorial-policy`} className="rounded-xl border border-border/70 bg-background/60 p-5 transition hover:border-accent/50">
              <div className="font-semibold text-foreground">{isTh ? "นโยบายบรรณาธิการ" : "Editorial policy"}</div>
            ...
            <Link href={`${localePrefix}/benchmark-summary`} className="rounded-xl border border-border/70 bg-background/60 p-5 transition hover:border-accent/50">
              <div className="font-semibold text-foreground">{isTh ? "สรุป Benchmark" : "Benchmark summary"}</div>
            ...
            <Link href={`${localePrefix}/evaluation`} className="rounded-xl border border-border/70 bg-background/60 p-5 transition hover:border-accent/50">
              <div className="font-semibold text-foreground">{isTh ? "ศูนย์กลางการประเมิน" : "Evaluation hub"}</div>
